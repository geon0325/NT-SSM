...
class NCL(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        ...
        self.config_name = f'{conf.dataset}_{conf.model_name}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_ssl{self.ssl_reg}_prt{self.proto_reg}_tau{self.ssl_temp}_alpha{self.alpha}_k{self.k}_nl{self.n_layers}'
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                if self.loss_type == 'bpr':
                    ...
                    rec_loss = self.ssm_loss(user_emb, pos_item_emb, self.tau)
                elif self.loss_type == 'directau':
                    rec_loss = self.direct_au_loss(user_emb, pos_item_emb, self.gamma)
