...
def next_batch_pairwise_bidir(data,batch_size,n_negs=1):
    training_data = data.training_data
    shuffle(training_data)
    ptr = 0
    data_size = len(training_data)
    while ptr < data_size:
        ...
        yield u_idx, k_idx, i_idx, j_idx

def next_batch_pairwise_ours(data,batch_size,n_negs=1):
    training_data = data.training_data
    shuffle(training_data)
    ptr = 0
    data_size = len(training_data)
    while ptr < data_size:
        ...
        items = [training_data[idx][1] for idx in range(ptr, batch_end)]
        ptr = batch_end
        u_idx, i_idx, j_idx, jj_idx, k_idx, l_idx = [], [], [], [], [], []
        item_list = list(data.item.keys())
        user_list = list(data.user.keys())
        for i, user in enumerate(users):
            i_idx.append(data.item[items[i]])
            ...
            for m in range(n_negs):
                ...
                neg_item = choice(item_list)
                while neg_item == items[i]:
                    neg_item = choice(item_list)
                k_idx.append(data.item[neg_item])
                
                neg_user = choice(user_list)
                while neg_user == user:
                    neg_user = choice(user_list)
                l_idx.append(data.user[user])
            jj_idx.append(j_idx[-1])
                
        yield u_idx, i_idx, j_idx, jj_idx, k_idx, l_idx
