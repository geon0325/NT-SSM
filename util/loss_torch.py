...
def bpr_loss(user_emb, pos_item_emb, neg_item_emb, alpha=1.0):
    pos_score = torch.mul(user_emb, pos_item_emb).sum(dim=1)
    neg_score = torch.mul(user_emb, neg_item_emb).sum(dim=1)
    loss = -torch.log(10e-6 + torch.sigmoid(pos_score - alpha * neg_score))
...
def InfoNCE_deg(view1, view2, degrees, temperature: float, b_cos: bool = True):
    if b_cos:
        view1, view2 = F.normalize(view1, dim=1), F.normalize(view2, dim=1)
    ...
    pos_score = torch.diag(score)
    neg_score = torch.sum(score, dim=1) - pos_score
    
    neg_score = (neg_score / degrees) + pos_score
    ...
    degrees = degrees / torch.sum(degrees) * len(degrees)
    score = score * degrees
    
    return -score.mean()


def InfoNCE_tau(view1, view2, degrees, temperature: float, b_cos: bool = True):
    if b_cos:
        view1, view2 = F.normalize(view1, dim=1), F.normalize(view2, dim=1)

    pos_score = (view1 @ view2.T) / temperature
    
    pos_score = torch.diag(degrees) @ pos_score
    score = torch.diag(F.log_softmax (pos_score, dim=1))
